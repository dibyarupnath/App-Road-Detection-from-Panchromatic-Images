...
class WindowAttention(nn.Module):
    def __init__(self, dim, heads, head_dim, shifted, window_size):
        ...
        self.logit_scale = nn.Parameter(
            torch.log(10 * torch.ones((1, self.heads, 1, 1, 1))), requires_grad=True
        )  # 1 h 1 1 1
        ...
        self.relative_coords_table = nn.Parameter(
    ...
    def forward(self, x):
        ...
        q, k, v = map(
            lambda t: rearrange(
                t,
                "b (nw_h w_h) (nw_w w_w) (h d) -> b h (nw_h nw_w) (w_h w_w) d",  # b h (nw_h nw_w) (w_h w_w) d
                h=h,
        ...
        dots = f.normalize(q, dim=-1) @ f.normalize(k, dim=-1).transpose(
            -2, -1
        )  # b h (nw_h nw_w) (w_h w_w) (w_h w_w)
        logit_scale = torch.clamp(
            self.logit_scale, max=torch.log(torch.tensor(1.0 / 0.01, device=x.device))
        ).exp()
        ...
        relative_position_bias = (
            relative_position_bias_table[self.relative_position_index.view(-1)]
            .view(
            ...
            .unsqueeze(0)
            .unsqueeze(0)
        )  # 1 1 (w_h w_w) (w_h w_w) h
        relative_position_bias = relative_position_bias.permute(
            0, 4, 1, 2, 3
        ).contiguous()  # 1 h 1 (w_h w_w) (w_h w_w)
        relative_position_bias = 16 * torch.sigmoid(
            relative_position_bias
        )  # 1 h 1 (w_h w_w) (w_h w_w)
        ...
        out = torch.einsum(
            "b h w i j, b h w j d -> b h w i d", attn, v
        )  # b h (nw_h nw_w) (w_h w_w) d
        out = rearrange(
            out,
            "b h (nw_h nw_w) (w_h w_w) d -> b (nw_h w_h) (nw_w w_w) (h d)",  # B, H, W, C_in
            h=h,
            w_h=self.window_size,
            w_w=self.window_size,
            nw_h=nw_h,
...
class SwinTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        channels=3,
        head_dim=32,
        window_size=7,  # channels = 1 for panchromatic images and we got to change the
