...
class CombinedLoss(nn.Module):
    def __init__(
        self, weight_bce=1.0, weight_dice=1.0, weight_iou=1.0, weight_road=2.0
    ):
    ...
    def forward(self, y_pred, y_true):
        ...
        loss = (
            ...
            + self.weight_road * road_loss
        ) / (self.weight_bce + self.weight_dice + self.weight_iou + self.weight_road)
        return loss
    ...
    def road_loss(self, y_true, y_pred, smooth=1e-6):
        ...
        road = (torch.sum(y_true_flat * y_pred_flat) + smooth) / (
            torch.sum(y_true_flat) + smooth
        )
        return 1 - road
